...
export default function TestCaseFilter({
    ...
    const handleApplyFilter = () => {
        ...
        if (selectedPriorities !== 'all' && selectedPriorities.size > 0) {
            priorityIndices = Array.from(selectedPriorities).map((key) =>
                priorities.findIndex((priority) => priority.uid === key)
            ).filter((index) => index !== -1);
        }

        let typeIndices: number[] = [];
        if (selectedTypes !== 'all' && selectedTypes.size > 0) {
            typeIndices = Array.from(selectedTypes).map((key) =>
                testTypes.findIndex((type) => type.uid === key)
            ).filter((index) => index !== -1);
        }
                ...
                <Dropdown>
                    ...
                    >
                        {testTypes.map((type) => (
                            <DropdownItem
                                key={type.uid}
                                textValue={testTypeMessages[type.uid]}
                                className="flex items-center"
                            >
